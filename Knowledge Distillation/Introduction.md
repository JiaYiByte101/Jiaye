...
###  FitNets: 中间层提示指导（Hint-based Training）

FitNets 是知识蒸馏领域中第一个尝试利用教师模型“中间层特征”来指导学生模型训练的方法。

- **核心思想**：除了模仿最终输出，还让学生模型在中间层模仿教师模型的隐藏特征（hint layer）。
- **过程分两阶段**：
  1. **Hint Training**：学生通过一个回归器对齐中间层表示；
  2. **Soft Targets Training**：再通过 softmax 预测对齐教师的输出分布。

 FitNets 能显著提高学生模型的收敛速度和表达能力，尤其在学生模型更深但更窄的结构中表现出色。

---

### Self-Distillation: 自我蒸馏（无教师模型）

Self-Distillation 是一种无需外部教师模型的蒸馏方式，最早由 Zhang et al., 2019 系统化提出。

- **核心思想**：模型本身在不同训练阶段担任“教师”与“学生”的角色，例如：
  - 当前 epoch 的输出作为下一轮的 soft targets；
  - 或通过 EMA（指数滑动平均）构建教师网络。

- **代表方法**：
  -  DINO（Caron et al., 2021）：无监督表征学习中广泛采用；
  -  BYOT、CS-KD 等方法也属于自蒸馏范畴。

 自蒸馏常用于无标签任务、自监督学习与提升模型鲁棒性。
